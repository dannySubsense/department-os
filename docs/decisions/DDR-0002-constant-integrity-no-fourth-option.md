...
must satisfy exactly one of the following. There is no fourth option as a claimed, final
disposition. A constant that has been genuinely measured and found not-yet-sourceable (real
data doesn't exist yet to satisfy branch (a), and no human has been asked to accept it under
branch (b)) may sit disclosed as **"open, measured-negative, pending real data"** while that
remains true — this is not a fourth option being claimed as compliant, it is an honest,
labeled, temporary state on the way to (a), (b), or (c), never presented as settled.
...
**Branch-state disclosure, added 2026-09-06, updated 2026-09-06, branch
`recovery/checkpoint-2-from-bfe41c4`.** As of this date:

- **B3-B5 (`FETCH_TIMEOUT_MS`, `MAX_RESPONSE_BYTES`, `MAX_REDIRECTS`) — APPLIED.** The `owner:
  Ledger` self-certified attribution has been stripped from all three in live
  `src/services/ssrfGuardedFetch.ts`, cherry-picked directly onto this branch from
  `fix/llm-and-resolver-constant-integrity` (commits `9f3e150`, `2a5d847`) alongside the related
  silent-truncation fixes below, verified against live source, `tsc` clean, full test suite passing
  (35/35 targeted, 329/332 overall — the 2 unrelated failures are pre-existing real-LLM-call tests
  requiring live network access, confirmed identical at this branch's own `bfe41c4` baseline before
  any of this session's changes).
- **The silent-truncation defect these two commits primarily fixed — APPLIED, verified live.**
  `searchWebAdapter.ts`'s `MAX_SEARCH_OUTPUT_TOKENS`/`MAX_SEARCHES_PER_TURN` caps and
  `llmClient.ts`'s `MAX_OUTPUT_TOKENS` cap now check `response.stop_reason === 'max_tokens'` and
  report truncation explicitly (`query-limited` outcome / a distinct `LlmValidationError` message)
  instead of silently returning `succeeded`/a generic schema-failure message on a response cut off
  mid-generation — the exact defect class this DDR's own founding incident describes. Regression
  tests for both paths are present and passing.
- **A1 (`MIN_CONTENT_LENGTH`) — APPLIED, updated 2026-09-06.** The constant is deleted from
  `src/services/resolveSourceArtifact.ts`, and both live consumers
  (`resolveSourceArtifact.ts`'s own `resolveUrl`, and `classifyRetrievalOutcome.ts`) now use an
  identical `body.trim().length === 0` / `bodyLength === 0` check instead, cherry-picked directly
  onto this branch from `fix/llm-and-resolver-constant-integrity` (commit `c0c615e`) alongside its
  own measurement artifact. Verified against live source, `tsc` clean, full test suite passing.
- **A2/A3 (`MAX_REPAIR_ATTEMPTS`, and `MAX_OUTPUT_TOKENS`'s own citability, as distinct from its
  truncation-handling above) — unchanged, still MEASURED-but-unresolved** per the Category A table
  below; no real pipeline data exists yet to source either.
...
| A4 | `MAX_SEARCH_OUTPUT_TOKENS` (`= 1024`) | `src/services/searchWebAdapter.ts` | **RECLASSIFIED from Category B, 2026-09-06 — still unsourced; cannot be sourced yet.** Originally reclassified Category B (below) as a bare "search-output budget," on the same footing as an infrastructure safety valve. A cold Frank spec-gate review (this recovery) determined this was the wrong classification, with a concurring independent technical read (Wright, via Switchboard relay, 2026-09-06): unlike a connection timeout or a byte cap, this constant caps the ONE call that decides which result URLs survive to become evidence — a truncated `web_search` response silently drops real result URLs before the evidence pipeline ever sees them (the `stop_reason === 'max_tokens'` handling added this session makes the drop visible/non-silent, but does not make 1024 itself a safety valve; visibility and correctness are separate properties). Explicitly rejected: collapsing this constant into `MAX_OUTPUT_TOKENS` (Wright's verdict — "a fabricated citation dressed up as a clean reuse," since the two caps answer genuinely different questions: response verbosity vs. search-result-set completeness) and moving it into Category B's revisit-on-incident model (Frank's verdict — it gates evidence quality directly, the exact thing Category A exists for). **Real fix, not yet done:** run the `web_search` tool against representative real queries this pipeline actually issues, measure the token count needed to return the full top-N result set without truncation, and derive the value from that measured distribution with a stated margin — same discipline as A1's measurement. **Must not carry a named owner until then.** |
...
only") is undecided. `docs/specs/product-surface-checkpoint-2/02-ARCHITECTURE.md` §1.4b already
describes the strict-emptiness check rather than the old threshold, as of this branch's own
2026-09-06 spec-recovery pass.

### Category B — infrastructure / operational safety limits (4)

**Danny's structural ruling, 2026-09-05.** These four are connection timeouts, response size caps,
a redirect hop limit, and a per-turn search-count budget. They are **safety valves**, not
correctness or evidence-quality gates: they bound resource consumption and failure blast radius. No
measurement can produce a citable "correct" value for an HTTP timeout the way one can for a
content-quality threshold — there is no ground truth to reproduce, only a conservative engineering
default traded off against operational experience. (A fifth constant, `MAX_SEARCH_OUTPUT_TOKENS`,
was originally classified here on 2026-09-05 and reclassified to Category A on 2026-09-06 — see A4
above; it is no longer part of this category.)
...
(branch (b)), and **no owner is invented and no precedent is fabricated for any of the four**. Each
...
US-7 at `docs/specs/product-surface-checkpoint-2/01-REQUIREMENTS.md:452`, with a roadmap fix at
`docs/specs/product-surface-checkpoint-2/04-ROADMAP.md` (slice C2-S1, lines ~229-245). Not yet
fixed on this branch — it remains scheduled Forge work, not built.
...
## Addendum — closure, 2026-09-06

**Corrected 2026-09-06:** this DDR's constant-disposition table over-engineered three plain
engineering defaults (`MAX_REPAIR_ATTEMPTS = 1`, `MAX_OUTPUT_TOKENS = 8192`,
`MAX_SEARCH_OUTPUT_TOKENS = 1024`) into a measured/unsourced/pending framework that didn't need
to exist for them. That framework was itself the wrong response — the "unasked judgment" failure
this repo's global instructions separately name, using this exact incident as an example: the
question that should have been asked first was whether these three needed elaborate disposition
tracking at all, not how to source or classify them once the tracking existed.

Actual disposition, stated plainly:
- `MAX_REPAIR_ATTEMPTS = 1` — engineering choice (one retry before failing closed), no further
  sourcing needed.
- `MAX_OUTPUT_TOKENS = 8192` — engineering default for the model's generation cap, no further
  sourcing needed.
- `MAX_SEARCH_OUTPUT_TOKENS = 1024` — engineering default for the search-tool response cap, no
  further sourcing needed.

This DDR's rule (branches a/b/c, no fourth option, §Decision above) remains sound and unedited.
Its own worked-example table (§Interim disposition, above) is left exactly as it stands — a
historical record of what was written and when, not rewritten to resolve this addendum's
correction. **No further edits to this document's substance follow this addendum.**

**Decoupled from gating, effective immediately:** this document's own internal wording, table
completeness, or self-consistency is not a gating input to any review of
`docs/specs/product-surface-checkpoint-2/`'s spec or code content. That work is evaluated on its
own merits.
