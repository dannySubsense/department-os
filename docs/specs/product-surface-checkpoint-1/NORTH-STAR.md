...
**Status**: Locked
**Date**: 2026-08-19 (restored from git history at `9c166dc` on 2026-08-20 after the doc-cleanup
pass removed it from the live tree per Danny's instruction; content re-verified against the current
canonical docs before restoring, not blindly reinstated — see Traceability's restoration note)
...
time, with no directory or Department framing at all. This traces directly to
`01-REQUIREMENTS.md`'s Checkpoint 1 scope, which itself traces to
`docs/specs/product-surface/DESIGN-PROPOSAL.md`'s design gate (PASSED, `cfb793c`, merged PR #7).

## In Scope / Out of Scope

See `01-REQUIREMENTS.md`'s User Stories and Out of Scope sections for the enumerated boundary —
this sprint does not restate it, it points to it.
...
## Restoration note (2026-08-20)

This file was removed from the live tree in the doc-cleanup pass approved by Danny (commit
`5d3d10b`), then restored ahead of `/forge-start` when Forge's governance-load HALT check required
it. Before restoring, its content was checked against the current state of the four canonical
docs, not reinstated blindly: the Declared Intent and Success Criteria above remain accurate
verbatim — they predate, and are unaffected by, the substantive corrections made after the design
gate (the 4-group Active/Ready-Not-Started split, the Visual Direction section, the
Mission-Control-first roadmap resequence, the Vite config fix, and the legacy-link ruling replacing
the cancelled Investigation Workspace placeholder — none of which change this sprint's declared
intent or its four Layer-1 success criteria). Two dangling references to the since-removed
`INTAKE.md` were repointed to `01-REQUIREMENTS.md`, and the stale "(once written)" parenthetical
was removed now that `01-REQUIREMENTS.md` exists. No other content was changed.
