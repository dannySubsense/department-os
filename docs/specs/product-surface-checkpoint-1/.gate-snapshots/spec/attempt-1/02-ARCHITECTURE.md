...
**Traces to**: `01-REQUIREMENTS.md` (6 user stories; see its Acceptance Criteria section for
the count — not restated here per repo-wide no-manually-asserted-counts discipline), `INTAKE.md` §3, `INTERVIEW.md`,
...
In scope: three new Express JSON routes, three new read-model query functions, a React SPA with
three screens (Mission Control, Departments directory, Problem Department overview), Vite build
tooling, and the static-serving / dev-proxy integration boundary with the existing Express app.
...
|---|---|---|
| `getMissionControlView` | Assemble `MissionControlView` from `investigation`/`generation_run`/`brief_version`/`evidence_item`/`source_artifact` via 5 independent queries (§4) | `src/services/getMissionControlView.ts` (new) |
| `getDepartmentsView` | Assemble `DepartmentsView` (`DepartmentSummary[]`) from a static department list joined with one `investigation` existence/count check | `src/services/getDepartmentsView.ts` (new) |
...
| `App` (React root) | Client-side router (3 routes: `/`, `/departments`, `/departments/problem-department`) and top-level layout shell | `src/client/App.tsx` (new) |
...
**`getDepartmentsView`** — no investigation-level query beyond a single `EXISTS`/`COUNT` per
Department from `departmentRegistry` (static list of 4). Only Problem Department's `status` can
ever resolve `installed`; the other three are always `planned` literals (§7 "Department tile
installed/planned status ... NOT a persisted domain fact").

```sql
-- run once per DepartmentSummary.status === 'installed' department, to confirm the department has
-- a real backing service (Problem Department only, checkpoint 1) — not to compute activity/counts.
-- This is a smoke-check, not part of DepartmentSummary's shape (DepartmentSummary carries no
-- counts at all, per US-2 AC1's "name, thesis, status" only).
```
...
export const DEPARTMENTS: ReadonlyArray<DepartmentSummary> = [
  { id: 'problem-department', name: 'Problem Department',
    ...
    status: 'installed' },
  { id: 'signal-foundry', name: 'Signal Foundry', thesis: /* verbatim from compass §3 */ '', status: 'planned' },
  { id: 'prototype-department', name: 'Prototype Department', thesis: /* verbatim from compass §3 */ '', status: 'planned' },
  { id: 'creative-practice-engine', name: 'Creative Practice Engine', thesis: /* verbatim from compass §3 */ '', status: 'planned' },
] as const;
```

Open item (not a HALT — flagged for the human-facing spec review): the three `planned` Departments'
one-line thesis strings must be copied verbatim from `docs/product-architecture-and-direction.md`
§3 at implementation time — this architecture doc does not re-derive them to avoid a second,
possibly-drifted copy. The Problem Department thesis string above is already verified against
`DESIGN-PROPOSAL.md` §3's quoted text.

**`getMissionControlView`** — 5 independent queries (§8's `activeWork` mutual-exclusivity note:
...
-- case, and it falls through all three groups as originally defined (B-4 fix, this document):
...
Three React-Router routes, matching `DESIGN-PROPOSAL.md` §1's Checkpoint-1 subset exactly:
...
- **Production integration**: unchanged Express app serves `src/web/public/`'s built
  ...
  middleware needed for static serving. A catch-all route (`app.get('*', ...)`, registered AFTER
  the existing `/investigations/*` and new `/api/*` routes so it never shadows them) serves
  `index.html` for any of the 3 client-side routes on a hard page load/refresh, enabling
  client-side routing to resolve `/departments/problem-department` directly.
...
| `react-router-dom` | ^6.26.0 | Client-side route matching for the 3 screens (§6) |
...
1. `departmentRegistry`'s three `planned`-Department thesis strings must be copied verbatim from
   `docs/product-architecture-and-direction.md` §3 at implementation time (§5.3 note) — this
   document intentionally does not re-quote them to avoid a second, driftable copy.
2. No pagination/page-size limit exists anywhere in this design (§3's PROVISIONAL note) — flagged
