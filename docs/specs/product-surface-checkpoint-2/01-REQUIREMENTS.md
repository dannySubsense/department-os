...
- `generateBriefVersion` (`src/services/generateBriefVersion.ts`) already persists a real
  ...
  reimplements, only calls from a new endpoint. Its `supersedesVersionId` contract is verified
...
- One Express connector endpoint: `POST /api/investigations/:id/generation-runs`.
- One read endpoint: `GET /api/investigations/:id/workspace` (honest progress + status).
...
## Resolved Scope Corrections (Danny's ruling, 2026-08-22)
...
requirement text. (This is a separate ruling from H-1 — see `05-REVIEW.md`'s "Spec-Gate
Disposition" section, entry "H-1 — RESOLVED, Danny's binding ruling, 2026-08-24" — which concerns
...
- [ ] Submitting source(s) via "Start Investigation" calls the existing real
      `submitSources`/`resolveInvestigationSources` services (unchanged) and navigates the browser
...
- [ ] A second concurrent generation request for the same Investigation while one is already
      `'in-progress'` is rejected (e.g. `409`), not queued, not silently dropped, not double-run.
      No DB-enforced uniqueness constraint for this concern exists in live code today; this
      checkpoint must BUILD it — a new partial unique index (migration `009`, per
      `02-ARCHITECTURE.md` §1.1) enforcing at most one non-terminal `GenerationRun` per
      Investigation, so the guarantee holds even under a check-then-act race, not merely a
      pre-request application-level check.
...
- [ ] The endpoint calls the existing `generateBriefVersion` directly, in-process, synchronously
      awaiting its result — no queue, worker, or background-job abstraction is introduced.
- [ ] The endpoint catches and reports `generateBriefVersion`'s real typed outcomes
      (`BriefVersion` success, `BriefGenerationFailedError`, `InvalidSupersedeTargetError`,
      `StaleCorrectionConflictError`) distinctly — never collapses them into one generic error.
...
- [ ] Polling stops once the run reaches a terminal persisted outcome (success, failed) — verified
      ...
      Polling also stops (or transitions to a non-misleading state) once a run is disclosed as
      stale/interrupted per the AC above, rather than polling indefinitely against a run that will
      never produce further progress.
...
Acceptance Criteria:
- [ ] A real `getBriefForReview` read service resolves `getInvestigation` →
      `ProblemBrief.currentVersionId` → the fully-resolved Brief content, from the existing
      persisted tables — no new brief-shaped write path is introduced.
...
- [ ] A later `BriefVersion` never inherits or replaces an earlier version's `Decision` — decisions
      remain bound to the specific version reviewed (see US-13 AC4 — this holds across corrective
...
- [ ] The workspace's decision-history surface (`DecisionHistoryBanner`, US-10's chronological
      decision list, extended) renders, without burying and without requiring scrolling/interaction
      to reveal: `priorDecisions` (plural), the current `getAssignedState` result when non-`'valid'`
      (`'challenged'`/`'invalidated'`), and `isSuperseded` with a link to the current version via
      `ProblemBrief.currentVersionId` when true. `isSuperseded` is a distinct structural fact (some
      other `BriefVersion` under the same `problemBriefId` names this one via
      `supersedesVersionId`), never conflated with assigned validity state.
...
- [ ] The Investigation Workspace lets the operator add a new source to a `'brief-generated'`
      ...
      (`submitSources`/`resolveInvestigationSources`, unchanged) — without leaving the workspace
      URL.
...
|---|---|---|---|---|
| 1 | `getInvestigation` → `ProblemBrief.currentVersionId` → `getBriefForReview` chain (S10, "Files" bullet 1) | RETAINED | Same read chain, not redesigned — confirmed unimplemented in current code, still the correct contract | US-9 |
...
| 5 | `priorDecisions`/`isSuperseded` per-`ClaimVersion` and `BriefVersion` returned by `getBriefForReview` (S10 Impl. Notes) | RETAINED | Unchanged data contract; rendering of non-`'valid'` state and supersession itself is US-12's concern (trace row 23) | US-9 (data), US-12 / trace row 23 (banner) |
...
| 23 | `DecisionHistoryBanner` surfacing `priorDecisions` and non-`'valid'` assigned state (S12 "Files"/Impl. Notes) | REVISED | The `priorDecisions`-surfacing half was always RETAINED via US-10; the non-`'valid'` assigned-state and `isSuperseded` halves, previously deferred pending rows 21/22, are now built alongside them — REVISED only in that the banner's location is this sprint's workspace surface, not a separate "Completed State" screen (same location revision as row 10), not in scope or content | US-12 |
...
**Summary of disposition:** 21 of 25 traced items RETAINED (in substance, several with a location
REVISION from "separate screen" to "one workspace URL"), 4 REVISED, 0 MOVED, 0 REMOVED outright.
No original requirement is silently dropped without a stated disposition. (Prior draft's count of
"20 RETAINED / 3 REVISED / 2 MOVED" both undercounted REVISED rows against the table's own labels
and reflected rows 21-22 as MOVED before Danny's 2026-08-22 ruling restored them to RETAINED —
corrected here against a fresh per-row recount of the table above, not carried forward.)
...
| Two browser tabs both trigger generation for the same Investigation concurrently | Second request rejected (e.g. `409`) by the new partial unique index (migration `009`, US-3 AC2); no duplicate `GenerationRun`, no silent overwrite |
...
| A run's owning process is confirmed gone (or no step progress for an operator-observable period) with no terminal `GenerationRun` outcome ever written | Workspace detects and discloses the run as stale/interrupted (US-4), never as indefinite "in progress"; polling does not continue indefinitely against it; the disclosure itself never mutates persisted workflow state |
...
- **No re-implementation or modification of `generateBriefVersion`'s internal pipeline** — this
  sprint calls it, it does not alter extraction/analysis/recommendation logic.
...
- Assumes: `generateBriefVersion`, `submitSources`, `resolveInvestigationSources`, and
  `transitionInvestigationStatus` remain correct and unmodified in their internal contracts except
  where this document's US-7/US-8 fixes require touching `ssrfGuardedFetch.ts` specifically (US-7)
  — if downstream architecture work finds this assumption false, that is a blocking finding to
