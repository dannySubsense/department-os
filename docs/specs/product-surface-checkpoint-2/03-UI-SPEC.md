...
| Research/Provenance Rail | **Version-scoped** (refetched with the displayed version's `GetBriefForReviewResult` and changes when the operator navigates to a different Brief version, Flow US-1 AC5): per-evidence excerpt, label, source, stance (from `ClaimVersionEvidenceRef`, not `EvidenceItem`), relevance note; contradicting evidence shown inline with supporting evidence, never hidden or in a separate collapsed tab; **`SearchScopeNotice`, filtered to the displayed `BriefVersion.generationRunId`'s own `webSearchQueries` (queries performed + failed/blocked retrievals for the run that actually produced the Brief on screen) — corrected 2026-09-05, Frank spec-gate finding 1: a prior revision classified this as version-independent, which would attribute a later correction run's searches to a Brief that never saw them, a misattribution this checkpoint's own provenance-honesty standard does not tolerate elsewhere; the filter is a `workspace.generationRuns.find(r => r.id === displayedVersion.generationRunId)` lookup, no new read model, since §3.2 already carries `webSearchQueries` per run.** **Version-independent** (whole-Investigation run history; sourced from `workspace.generationRuns`; does not change when navigating between Brief versions): fixed `CitationScopeNotice`; per-run runtime identifier/models/tools/steps for every run in `workspace.generationRuns` (not only the latest, rendered by `RunHistoryList`), each step's real `validationRecords`/`toolInvocations` fields, and each run's real `webSearchQueries` array (queries + per-result retrieved/blocked/failed status) as part of that whole-Investigation history — distinct from `SearchScopeNotice`'s own version-scoped, single-run filtered view above. | `GetBriefForReviewResult` (version-scoped evidence, plus `generationRunId` used to filter `SearchScopeNotice`), `workspace.generationRuns` (version-independent `RunHistoryList` content) |
...
3. Every region updates to reflect the target version: header's "Version N of M" indicator plus
 ...
 refetched as part of `GetBriefForReviewResult` — version-SCOPED content), region 4's
 `SearchScopeNotice` (corrected 2026-09-05, Frank spec-gate finding 1 — filtered to the newly-
 displayed version's own `generationRunId`, changes on navigation, same as `EvidenceProvenanceList`),
 and the Decision Area's per-version `priorDecisions` list and Approve/Reject/Watch
 controls (bound to that version's `briefVersionId`). Region 4's remaining content — `RunHistoryList`
 (all `GenerationRun`s and their steps) and `CitationScopeNotice` — does NOT
...
region 4's `EvidenceProvenanceList`/`SearchScopeNotice` (the version-dependent content) — regions
1/2 and region 4's `RunHistoryList`/`CitationScopeNotice` do not re-flash, since that content is
...
└── InvestigationWorkspaceScreen (routes: /departments/problem-department/
 ...
 │ │ VERSION-SCOPED, filtered to the displayed
 │ │ version's own generationRunId; corrected
 │ │ 2026-09-05, Frank spec-gate finding 1 — changes
 │ │ on version navigation, same as
