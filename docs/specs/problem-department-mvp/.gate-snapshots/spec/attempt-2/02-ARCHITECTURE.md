...
/** Carrier for "explicitly recorded absence" — 01's Required Brief Elements rule (line 19):
 *  each of these five elements must be either populated (its id array non-empty) OR carry an
 *  explicit, non-empty absence statement; a structurally empty array is never an acceptable
 *  substitute for either. Kept as a small entity keyed by (briefVersionId, element) rather than
 *  five nullable fields on BriefVersion, so the shape generalizes uniformly across Problem
 *  Statement, Evidence, Demand Signal Type, Existing-Solution Landscape, and Gap Hypothesis —
 *  the same five elements 03-UI-SPEC.md's NegativeFindingNotice renders (G-18). Evidence and
 *  Demand Signal Type deliberately do NOT gain a "none-found" member on their respective closed
 *  unions (EvidenceLabel, DemandSignalType) — those unions describe what WAS found; absence is
 *  recorded here instead, one level up, exactly as Existing-Solution Landscape and Gap
 *  Hypothesis already do via their empty id arrays plus this same carrier. */
type BriefElement =
  | 'problem-statement'
  | 'evidence'
  | 'demand-signal-type'
...
interface BriefVersion {
  ...
  negativeFindings: NegativeFinding[]; // 0..5 rows — one per element in this list that had
                                     // nothing to report (Problem Statement, Evidence, Demand
                                     // Signal Type, Existing-Solution, Gap Hypothesis); an
...
// all seven required elements populated — where "populated" for each of the five elements
...
| **Element-level negative-finding carrier, not a type-level escape hatch** | `NegativeFinding` (keyed by `briefVersionId`, `element`) records "nothing found" one level above `DemandSignalType`/`EvidenceLabel`/etc.; those closed unions gain no `'none-found'` member, and `DemandSignal.evidenceItemIds`/`ExistingSolution.evidenceItemIds`/`GapHypothesis.evidenceItemIds` keep their `NonEmptyArray` typing unweakened | G-1 fix — a `'none-found'` union member or a relaxed `NonEmptyArray` would have reopened the exact empty-citation loophole the R-4 fix closed; the absence statement needs its own field, not a hole in the citation rule |
