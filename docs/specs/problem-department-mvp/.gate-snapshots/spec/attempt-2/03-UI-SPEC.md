...
│    who / context / consequence, per statement, OR the       │
│    negativeFindings entry for element                       │
│    'problem-statement' (its .statement text) — never a       │
│    blank section (G-18)                                      │
├─────────────────────────────────────────────────────────┤
│ 2. Evidence                                                 │
...
of the five Brief elements that can legitimately have zero populated items — Problem Statement,
...
`getBriefForReview().negativeFindings` (top-level, `NegativeFinding[]`, 0–5 rows) contains an
...
these five elements has either its own populated content (e.g. `problemStatements[]` non-empty)
or a matching `negativeFindings` entry — never neither. This is categorically different from a
true UI-loading-empty-state (e.g. the Investigation Screen has not yet resolved data from the
backend, or a network error occurred):
...
| Problem Statement(s) | `whoExperiencesIt`, `contextOrWorkflow`, `consequenceOrFriction` per `ProblemStatement`, or the `negativeFindings` entry with `element === 'problem-statement'` | `problemStatements[]`, `negativeFindings[]` |
...
└── CompletedState (conditional on status === 'brief-generated')
    ...
    │   └── ProblemStatementItem (×n) | NegativeFindingNotice   // keyed off negativeFindings
    │                                         // entry with element === 'problem-statement'
    ├── EvidenceSection
...
legitimately have zero populated items. These five map exactly onto architecture's `BriefElement`
union (`'problem-statement' | 'evidence' | 'demand-signal-type' | 'existing-solution' |
'gap-hypothesis'`); each rendered `NegativeFindingNotice` looks up the `negativeFindings` entry
(if any) whose `.element` matches its own section's `BriefElement` value and displays that
entry's `.statement`. The remaining **2 of the seven**, Uncertainty and Decision Recommendation,
are structurally always-populated, are not members of `BriefElement`, and use no
`NegativeFindingNotice`: Uncertainty always renders its three named lists, and Decision
Recommendation always produces an Approve/Reject/Watch determination with rationale — even when
that rationale is driven by insufficient or absent evidence (e.g. a Watch recommendation citing
"no defensible gap established"), the recommendation field itself is never absent, so it has no
`negativeFindings` case to key off. (Demand Evidence's confidence sub-field,
`DemandConfidenceSection`, is likewise always-populated on its own — the element as a whole
still falls in the 5-of-7 group because its signal-type sub-field, `demand-signal-type`, is a
`BriefElement` value and can have a `negativeFindings` entry.)
...
- [x] Every one of the seven required Brief elements — (1) Problem definition, (2) Evidence,
      ...
      `NegativeFindingNotice` keyed off a matching `negativeFindings[]` entry (by `.element`)
      when empty; the remaining 2 (Uncertainty, Decision Recommendation) are structurally
      always-populated, are not `BriefElement` members, and carry no negative-finding case at
      all rather than carrying it in narrative content; every one has a stated, structural
