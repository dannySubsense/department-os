...
| Evidence/Claim schema (`Claim`/`ClaimVersion`/`EvidenceItem`) + Extraction & Clustering Engine + Evidence Labeler | Source Resolver (needs reachable sources) |
...
| Validity/Invalidation Service (`StatusEvent`, `assignValidityState`, `getAssignedState`) + Decision-History Banner (Flow 5) | Brief Assembler, Decision Recorder |
...
**Files:**
- Persistence for `Claim`, `ClaimVersion`, `EvidenceItem`, `ProblemStatement` (Architecture §3
  "Evidence & Claims" — Q-4 stable-identity/immutable-version split)
...
- Contradicting evidence recorded via `stance: 'contradicting'`, not omitted.
...
- [ ] Given multiple sources with overlapping evidence, evidence items supporting the same claim
      are grouped under the same `ClaimVersion`.
- [ ] Given contradicting evidence in source material, it is recorded with `stance:
      'contradicting'`, present in the Brief's eventual evidence set, not dropped.
...
- [ ] Given Personal-Pull-only source material, `DemandSignal` records an explicit "no demand
      signal types were found" negative finding (never a structurally empty list, per Q-2/G-18)
      and `DemandConfidenceClassification.level` is `Insufficient` with a narrative explaining the
      absence (Edge Case row).
...
- `SourceArtifact.origin: 'landscape-research'` write path — a web result the Landscape
  ...
  through the existing `EvidenceItem`/`ClaimVersion` pipeline (Slice 4) unchanged; no second
  citation model
...
- [ ] Given a search that returns zero results or whose retrievals fail/are blocked, the
      ...
      `limitations`) and the Brief still records the search scope and an explicit "none found"
      landscape finding — not a workflow failure (Edge Case row).
...
generation-run failure — never silent coercion — if repair is exhausted.
...
- Schema-validation/bounded-repair mechanism implementation — invoked by the Provenance Recorder
  ...
  note); the specific validation library is an implementation choice within whatever Slice 1
  selected, not specified here
...
- Every `GenerationStep` that produces one or more schema-constrained structured outputs (a
  ...
  unset.
- On validation failure, exactly one bounded repair attempt is made
  (`MAX_REPAIR_ATTEMPTS = 1`, configuration not hardcoded per call site) — re-prompt the model
  with the original output and the validation error, then re-validate. No open-ended retry loop.
...
- If the repaired output still fails validation, the step is terminal-failed
  (`SchemaValidationRecord.finalOutcome === 'invalid'`): it produces no usable `outputRef` for
  that field, and this composes into `GenerationRun.outcome: 'failed'` — never silently coerced,
  defaulted, or persisted as if valid (Anti-Patterns table, R-4).
...
**Done When:**
- [ ] US-11 acceptance criteria pass, including the schema-validation/bounded-repair AC.
- [ ] All tests pass.
...
Screen — Generation Failed State component built in Slice 3 (N-4).

**Depends On:** Slice 8

**Files:**
- Persistence for `ProblemBrief`, `BriefVersion` (Architecture §3 "Problem Brief identity &
  versioning")
...
- A `GenerationRun.outcome: 'failed'` produced by Slice 8's bounded-repair exhaustion (a
  schema-validation terminal failure) is one of the failure paths that must surface as
  `Investigation.status === 'generation-failed'` here — this slice does not need its own
  validation logic, only to honor Slice 8's recorded `outcome` when deciding the Investigation's
  resulting status.
...
- [ ] Given a correction, a new `BriefVersion` is created with `supersedesVersionId` pointing at
      the prior version; the prior version's content remains readable unchanged.
...
- [ ] Given a `GenerationRun.outcome: 'failed'` produced by exhausted schema-validation repair
      (Slice 8), `Investigation.status` becomes `'generation-failed'` and no `BriefVersion` is
      created.
...
- The Existing-Solution Landscape section (section 4) must render `SearchScopeNotice` — the
  ...
  found (Q-6).
- `NegativeFindingNotice` applies to exactly 5 of the 7 required Brief elements — Problem
  Definition, Evidence, Demand Evidence (the signal-type field), Existing-Solution Landscape, and
  Gap Hypothesis — each of which must render this component's populated, explicit absence
  statement (structurally distinct from a loading/error empty state) when the corresponding field
  is a recorded negative finding, not just when populated (Q-2/G-18). Demand Evidence's confidence
  field and Uncertainty are structurally always-populated and never render `NegativeFindingNotice`
  — see 03-UI-SPEC.md.
...
- [ ] `SearchScopeNotice` renders the queries performed and any failed/blocked retrievals in the
      ...
      found (Q-6).
- [ ] For each of the 5 sections that render `NegativeFindingNotice` (Problem Definition,
      Evidence, Demand Evidence's signal-type field, Existing-Solution Landscape, Gap Hypothesis),
      given a `BriefVersion` whose corresponding field holds a negative-finding value,
      `NegativeFindingNotice` renders as a completed, non-error, non-loading statement in the
      section's normal layout position — never sharing visual/structural treatment with a true
      loading/error empty state (G-18).
...
- `assignValidityState`, `getAssignedState` service functions (Architecture §4)
...
- **No mutable status field anywhere** — `Claim`, `ClaimVersion`, and `BriefVersion` carry no
  `status` column. Assigned validity state is answered by one query: **the latest `StatusEvent`
  for `(targetType, targetId)` with `effectiveAt <= asOf`** (ordered by `effectiveAt`, then
  `recordedAt` as a tie-break), defaulting to `'valid'` when no `StatusEvent` exists yet. This is
  the query `getAssignedState` implements, and it is now computable — the "not computable at
  markedAt" caveat from the prior version of this document no longer applies; the append-only
  `StatusEvent` log (Q-3, closing G-3) is exactly what makes "what state did we assign at time T"
  answerable without a mutable field.
- Framing discipline (Q-3, binding): this answers *"what validity state did Department OS assign
  to this item at time T,"* never *"was this item objectively valid at time T."* API/UI naming
  and copy must preserve this — `getAssignedState`, not `getValidityAt`/`isValid`.
- `assignValidityState`'s `dependentDecisionIds` (US-10 AC3's "whole point") is computed by: (1)
  ...
  assigned state — per `getAssignedState` evaluated at each `Decision.decidedAt` — was last
  `'valid'`. Computed at call time, never stored redundantly.
...
- [ ] Given a `ClaimVersion`/`BriefVersion` marked `invalidated`, `dependentDecisionIds` includes
      ...
      target's assigned state was last `'valid'`.
...
- [ ] Given a late-discovered correction (a `StatusEvent` with `effectiveAt` in the past but
      `recordedAt` now), `getAssignedState` for a `asOf` between the two timestamps reflects the
      new event, without mutating any prior event.
...
- [x] `getInvestigation` (G-20/G-15), the `StatusEvent`-backed validity query (G-21), Q-6's
      independent web-search capability (Slice-6 scope), Slice 1's Q-8 framing and G-22
      stopping rule, the `GenerationRun`/`Investigation.status`/`priorDecisions` schema
      updates (G-12/G-13/G-11), and R-4's schema-validation/bounded-repair mechanism (Slice 8)
      are all reflected in the slice detail above.
- [x] All UI vocabulary throughout this document matches `03-UI-SPEC.md`'s current model exactly:
      ...
      document (F-3).
- [x] `SearchScopeNotice` and `NegativeFindingNotice` (5 of the 7 required Brief elements, per
      `01-REQUIREMENTS.md`'s canonical enumeration) have explicit test coverage in Slice 10's
      Tests, and Slice 10's Implementation Notes state both render regardless of whether the
      corresponding section found a populated result (F-5).
