...
- [ ] Slice 9: Brief Assembler — **BLOCKED / NOT STARTED.** A first implementation attempt was made against a design that subsequently FAILED Composer QC with 7 blocking + 4 significant findings; it is QUARANTINED at `quarantine/slice-9-attempt-1` (`86f7b8b`) and is explicitly NOT evidence of anything — its tests are not coverage. Design is at **revision 8** (see the revision history in `SLICE-09-DESIGN.md`; four Composer design gates have been run against it, each returning real defects). Revision 4 failed the Composer's design gate with 7 blocking + 4 significant findings; revision 5 corrected those and added the G-1 precedence ruling (a component failure means "unknown because generation failed," not "searched and found nothing," so it cannot legally produce a NegativeFinding and hard-stops the run), then **failed a second design gate** with 5 further blocking defects: retry from `generation-failed` could not complete (a committed Brief with a stuck Investigation, because the transition's return value was ignored); the stale-race taxonomy was unimplementable from a single read (no pre-pipeline snapshot, so an already-stale request and a genuine race were indistinguishable); the ownership query passed a ClaimVersion citing one local AND one foreign EvidenceItem; persisted nested objects omitted architecture-required binding fields; and the finalization contracts conflicted such that a literal implementation would double-finalize. Revision 6 corrected all five and was delivered as a three-document contract — `SLICE-09-DESIGN.md`, `04-ROADMAP.md`, and `02-ARCHITECTURE.md` §1.9 — swept together rather than individually. **Revision 6 then failed a third gate, narrowly**, on two contract defects: (a) preflight verified the supersede target belonged to the ProblemBrief but never checked it was the CURRENT version, so a caller supplying v1 while current is v3 passed both preflight and the phase-4 race check — the two-observation race mechanism detects *change*, and nothing changed, so the branch was invisible to it; (b) the "this-run evidence" ownership rule was incompatible with the components being orchestrated — `demandAnalyzer`, `personalPullExtractor` and `landscapeResearcher` read ALL persisted Investigation evidence via `getEvidenceForInvestigation`, so a legitimate correction citing older local evidence would fail phase 3, making correct runs impossible. **Revision 7** patches exactly those two contracts (diff confined to the revision note, module plan, phase-2 preflight, and phase-3 check 1 — everything the Composer accepted is untouched) and adopts, as a flagged Producer decision open to override, provenance = the start-of-run evidence snapshot UNION evidence created during the run, rather than changing four merged slices' signatures. Revision 7 then failed a fourth gate on ONE defect: `startSnapshot` is captured before the Extraction step runs, but the permitted union omitted `ExtractionResult.evidenceItems` — so a candidate citing evidence created by this run's own Extraction step (the PRIMARY path, not an edge case) would have failed phase 3. **Revision 8** corrects all seven references to `startSnapshot ∪ ExtractionResult.evidenceItems ∪ LandscapeResearchResult.landscapeEvidenceItems` and adds a falsification test covering a normal first generation citing Extraction-created evidence. The Composer ACCEPTED both revision-7 Producer decisions (snapshot-union over changing four merged component signatures; `claimVersionId` stays restricted to this run's extraction). Awaiting the Composer's re-gate of that formula alone before reimplementation.
...
blocked pending Composer review of `SLICE-09-DESIGN.md` revision 8. The prior attempt is
