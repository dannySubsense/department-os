...
| **Generation Failed** | `Investigation.status === 'generation-failed'` | reason (sources were reachable, but the pipeline could not produce a valid Brief — most commonly, no Problem Statement could be established from the material; also covers any other required-element/schema-validation failure) + a *different* remedy (no "add a source" control — see below) |
...
1. User starts at: **Investigation Screen**, Generating state, having submitted at least one
   reachable source.
2. System response: the generation pipeline (`generateBriefVersion`) runs but fails to establish
   a required, non-negatable element — per architecture's binding Q-2 correction, this is
   overwhelmingly the case where no valid `ProblemStatement` can be established from the
   material (Problem Statement has no absence path: unlike the four negatable elements, a Brief
   is never completed with "no problem found" recorded as a finding) — or, less commonly, some
   other pipeline/schema-validation failure; `Investigation.status` becomes
   `'generation-failed'`; `GenerationRun` is persisted with `outcome: 'failed'` and
   `briefVersionId: null`; no `BriefVersion` is created.
...
│  complete"                                             │
├───────────────────────────────────────────────────┤
│ Reason: statusReason (most commonly: no problem could │
│ be established from the material; may also reflect     │
│ another required-element or schema-validation failure) │
├───────────────────────────────────────────────────┤
...
| Section | Content | Data Source |
|---------|---------|--------------|
| Reason statement | Plain-language statement distinct from Blocked: sources were reachable, the generation pipeline itself did not complete. Per architecture's binding Q-2 correction, this state is reached above all when no valid `ProblemStatement` could be established (Problem Statement is not a negatable `BriefElement` — its absence is never rendered as a Brief finding, it is always a hard generation failure); the copy also covers rarer non-problem-statement pipeline/schema failures | `Investigation.status === 'generation-failed'`, `statusReason` |
...
Because `Investigation.status` can only reach `'brief-generated'` once a valid `ProblemStatement`
has been established (architecture's binding Q-2 correction: Problem Statement has no absence
path — `generateBriefVersion` fails explicitly, leaving the Investigation
`'generation-failed'`, in any case where none can be established), the Problem Statement section
below **always renders real `ProblemStatement` content and never a `NegativeFindingNotice` or
any other absence/empty-state rendering** — by construction, a Completed Brief cannot exist
without at least one populated `ProblemStatement`.
...
│ "This Brief's citations are guaranteed to be present and    │
│ attributed to a specific evidence item for every claim,      │
│ signal, solution, and gap — the relevance/correctness of     │
│ what's cited has not been independently verified."           │
├─────────────────────────────────────────────────────────┤
│ 1. Problem Statement(s)                                    │
│    who / context / consequence, per statement — always      │
│    real content, never a NegativeFindingNotice or blank      │
│    section: Problem Statement is not negatable (Q-2), so a   │
│    Completed Brief always has ≥1 populated ProblemStatement  │
├─────────────────────────────────────────────────────────┤
│ 2. Evidence                                                 │
│    Grouped by claim (one group per ClaimVersion); each       │
│    group lists its ClaimVersionEvidence relationships        │
│    (ClaimVersion.evidence, resolved via getBriefForReview's   │
│    resolvedEvidence), each row showing: the linked            │
│    EvidenceItem's label badge (fact/observation/              │
│    interpretation/assumption/unknown), THIS relationship's    │
│    stance (supporting/contradicting/neutral-context — read     │
│    from the relationship, not the item), excerpt, source link,│
│    optional relevanceNote. Stance is per claim-evidence        │
│    relationship, not intrinsic to the evidence item: if the    │
│    same EvidenceItem is cited by two different claims with     │
│    different stances, each claim's group renders its own       │
│    relationship's stance independently — e.g. "supports"       │
│    under one claim and "contradicts" under another for the     │
│    same underlying item — never collapsed to one badge shared  │
│    across groups.                                              │
│    Contradicting evidence shown inline with supporting          │
│    evidence under the same claim.                               │
│    If no evidence was found at all: the negativeFindings        │
│    entry for element 'evidence' (its .statement text) —         │
│    (G-18) — not omission                                        │
├─────────────────────────────────────────────────────────┤
...
│    otherTypeLabel. If none found: the negativeFindings        │
│    entry for element 'demand-signal-type' (its .statement     │
│    text), not a blank list (G-18)                              │
...
│    documented search scope: the negativeFindings entry for     │
│    element 'existing-solution' (its .statement text)           │
...
│    If none established: the negativeFindings entry for         │
│    element 'gap-hypothesis' (its .statement text), not a        │
│    blank section (G-18)                                        │
├─────────────────────────────────────────────────────────┤
│ 6. Uncertainty                                               │
...
**Distinguishing recorded-negative-finding from true UI-empty-state (G-18, binding)**: for each
of the **four** Brief elements that can legitimately have zero populated items —
Evidence, Demand Signal Type, Existing-Solution, and Gap Hypothesis (architecture's
`BriefElement` union) — the `NegativeFindingNotice` for that section renders when
`getBriefForReview().negativeFindings` (top-level, `NegativeFinding[]`, 0–4 rows) contains an
entry whose `.element` matches that section's `BriefElement` value; the notice displays that
entry's `.statement` text verbatim. There is no separate "explicit absence statement" field
per Brief element — `negativeFindings`/`NegativeFinding.statement` is the one mechanism, keyed
by `element`. Per Requirements' Required Brief Elements rule, the fail-closed validation gate at
the architecture layer already guarantees that, for a `brief-generated` Investigation, each of
these four elements has either its own populated content (e.g. `existingSolutions[]` non-empty)
or a matching `negativeFindings` entry — never neither. **Problem Statement is explicitly
excluded from this set** — per architecture's binding Q-2 correction, `'problem-statement'` is
not, and must never be treated as, a member of `BriefElement`; it has no absence path, no
`NegativeFindingNotice` rendering, and no `negativeFindings` entry is ever constructed for it.
Any Investigation for which no valid `ProblemStatement` could be established fails generation
outright (`'generation-failed'`, see above) — no `brief-generated` Investigation, and therefore
no rendering of this Completed state, is ever reached in that case. This is categorically
different from a true UI-loading-empty-state (e.g. the Investigation Screen has not yet resolved
data from the backend, or a network error occurred):
- A **recorded negative finding** — i.e. a matching `negativeFindings` entry rendered via
  `NegativeFindingNotice` — appears as a plain, non-error, non-loading statement, styled
...
- A **true loading/error empty-state** (e.g. "Unable to load Brief," a spinner, a retry control)
  ...
  treatment with `NegativeFindingNotice` — conflating the two would let a human mistake "the
  system found nothing" for "the system is broken," or vice versa.
  ...
  noting the distinction must exist), never rendered using `NegativeFindingNotice`.

**Citation-scope notice (R-1 fix, fixed and always visible)**: per architecture's accepted MVP
limitation (R-1, `02-ARCHITECTURE.md` Section 4), the Brief Review surface must state — visibly,
not buried in a tooltip or footnote — that citation **presence** is guaranteed (every required
citation field is enforced non-empty by construction and runtime validation) but citation
**correctness/relevance** has not been independently verified by the system. This notice appears
once, near the top of the Completed state (below the provenance line, above section 1), and its
copy must never be read as implying required citations may be *absent* — presence is
structurally enforced, not merely declared; only correctness is disclaimed.
...
the same plus its `negativeFindings` entry for element `'existing-solution'`. This directly
prevents the Brief from reading as an exhaustive market scan when it is a scoped,
honestly-bounded search.
...
| Problem Statement(s) | `whoExperiencesIt`, `contextOrWorkflow`, `consequenceOrFriction` per `ProblemStatement` — always real content; Problem Statement is not a `BriefElement`, has no `negativeFindings` entry, and never renders `NegativeFindingNotice` (Q-2) | `problemStatements[]` |
| Evidence | `Claim.text` (via `ClaimVersion`) as group heading; nested per-relationship rows from `resolvedEvidence[]`, each showing the linked `EvidenceItem.label`/`excerptOrSummary`/source plus THIS relationship's own `stance`/`relevanceNote`; or the `negativeFindings` entry with `element === 'evidence'` | `claimVersions[]` (with embedded `resolvedEvidence: Array<ClaimVersionEvidenceRef & { item: EvidenceItem }>`, per `getBriefForReview`), `negativeFindings[]` |
| Demand Signal Types | `DemandSignal.type` (+ `otherTypeLabel`), linked evidence, or the `negativeFindings` entry with `element === 'demand-signal-type'` | `demandSignals[]`, `negativeFindings[]` |
| Demand Confidence | `level`, `narrative`, `citedDemandSignalIds` resolved to their signals | `demandConfidence` |
| Existing-Solution Landscape | `ExistingSolution` fields, or the `negativeFindings` entry with `element === 'existing-solution'`, plus search-scope/limitations note | `existingSolutions[]`, `negativeFindings[]`, `WebSearchQuery[]`/`WebSearchResult[]` for this `generationRunId` |
| Gap Hypothesis | `category` (+ `otherCategoryLabel`), `statement`, linked evidence, or the `negativeFindings` entry with `element === 'gap-hypothesis'` | `gapHypotheses[]`, `negativeFindings[]` |
...
│   │                                         // most commonly: no ProblemStatement established (Q-2)
│   ├── RetryNote
│   └── ResubmitSourceLink
└── CompletedState (conditional on status === 'brief-generated')
    ...
    │   └── ProblemStatementItem (×n)        // always real content — Problem Statement is not a
    │                                         // BriefElement, has no NegativeFindingNotice rendering
    │                                         // path (Q-2): a Completed Brief always has ≥1
    ├── EvidenceSection
    │   ├── ClaimGroup (×n)                  // one per ClaimVersion
    │   │   └── ClaimEvidenceRow (×n)        // one per resolvedEvidence[] entry (a
    │   │                                    // ClaimVersionEvidenceRef + its resolved EvidenceItem);
    │   │                                    // stance rendered here is this relationship's own —
    │   │                                    // the same EvidenceItem may appear in another
    │   │                                    // ClaimGroup with a different stance
    │   └── NegativeFindingNotice (conditional)  // keyed off negativeFindings entry with
    │                                             // element === 'evidence'
    ├── DemandSignalSection                  // Demand Evidence element, field 3a
    │   └── DemandSignalItem (×n) | NegativeFindingNotice   // keyed off negativeFindings
    │                                         // entry with element === 'demand-signal-type'
    ...
    │   ├── ExistingSolutionItem (×n) | NegativeFindingNotice   // keyed off negativeFindings
    │   │                                     // entry with element === 'existing-solution'
    │   └── SearchScopeNotice                // Q-6: queries run + blocked/failed retrievals
    ├── GapHypothesisSection
    │   └── GapHypothesisItem (×n) | NegativeFindingNotice   // keyed off negativeFindings
    │                                         // entry with element === 'gap-hypothesis'
...
`NegativeFindingNotice` appears within **4 of the seven** required Brief elements — Evidence,
Demand Evidence (specifically its signal-type sub-field, rendered by `DemandSignalSection`),
Existing-Solution Landscape, and Gap Hypothesis — each of which can legitimately have zero
populated items. These four map exactly onto architecture's `BriefElement` union
(`'evidence' | 'demand-signal-type' | 'existing-solution' | 'gap-hypothesis'`); each rendered
`NegativeFindingNotice` looks up the `negativeFindings` entry (if any) whose `.element` matches
its own section's `BriefElement` value and displays that entry's `.statement`. **Problem
Statement is not a `BriefElement` and renders no `NegativeFindingNotice`** — per architecture's
binding Q-2 correction, it has no absence path: `generateBriefVersion` fails explicitly
(`Investigation.status` → `'generation-failed'`) whenever no valid `ProblemStatement` can be
established, so a Completed Brief is never reached in that case and `ProblemStatementSection`
always renders real content. The remaining **2 of the seven**, Uncertainty and Decision
Recommendation, are likewise structurally always-populated, are not members of `BriefElement`,
and use no `NegativeFindingNotice`: Uncertainty always renders its three named lists, and
Decision Recommendation always produces an Approve/Reject/Watch determination with rationale —
even when that rationale is driven by insufficient or absent evidence (e.g. a Watch
recommendation citing "no defensible gap established"), the recommendation field itself is never
absent, so it has no `negativeFindings` case to key off. (Demand Evidence's confidence
sub-field, `DemandConfidenceSection`, is likewise always-populated on its own — the element as a
whole still falls in the 4-of-7 group because its signal-type sub-field, `demand-signal-type`,
is a `BriefElement` value and can have a `negativeFindings` entry.)
...
(SearchScopeNotice), `Review Surface` (CompletedState's read via `getBriefForReview`, including
its top-level `negativeFindings[]`), `Decision Recorder` (DecisionForm → `recordDecision`),
`Validity/Invalidation Service` (DecisionHistoryBanner's `assignedState`/`isSuperseded`).
...
| `problemStatements[]` (always ≥1 for a Completed Brief; not negatable, no absence path) | CompletedState — ProblemStatementSection | Brief Assembler (`generateBriefVersion`); read via `getBriefForReview` |
| `negativeFindings[]` (0–4 `NegativeFinding` rows, one per empty-eligible `BriefElement`: evidence, demand-signal-type, existing-solution, gap-hypothesis) | CompletedState — each of the four `NegativeFindingNotice` instances (Evidence, Demand Signal Type, Existing-Solution, Gap Hypothesis), keyed by `.element` | Brief Assembler (`generateBriefVersion`), persisted on `BriefVersion`; read via `getBriefForReview().negativeFindings` (top-level) |
...
| `EvidenceItem.label` | CompletedState — Evidence section (per `ClaimEvidenceRow`) | Evidence Labeler |
| `ClaimVersionEvidence.stance`/`.relevanceNote` (per claim-evidence relationship, resolved as `resolvedEvidence[]` on each `ClaimVersion`) | CompletedState — Evidence section (per `ClaimEvidenceRow`, scoped to its own `ClaimGroup` — the same `EvidenceItem` can render a different stance under a different claim) | Extraction & Clustering Engine / Evidence Labeler, at claim-evidence relationship creation time |
...
- [x] Every one of the seven required Brief elements — (1) Problem definition, (2) Evidence,
      ...
      explicit-negative-finding handling: 4 of the 7 (Evidence, Demand Evidence's signal-type
      field, Existing-Solution Landscape, Gap Hypothesis) render a `NegativeFindingNotice` keyed
      off a matching `negativeFindings[]` entry (by `.element`) when empty; the remaining 3
      (Problem Statement, Uncertainty, Decision Recommendation) are structurally
      always-populated, are not `BriefElement` members, and carry no negative-finding case at
      all rather than carrying it in narrative content — Problem Statement specifically has no
      absence path by design (Q-2, binding): a Completed Brief cannot exist without ≥1 valid
      `ProblemStatement`, and any Investigation that cannot establish one fails generation
      explicitly instead of reaching Completed; every negatable element has a stated, structural
      distinction from a true UI loading/error empty-state (G-18).
- [x] Blocked and Generation-Failed states are specified as distinct, with distinct remedies —
      "add a source" is never offered as the fix for a generation failure (G-13-derived); the
      Generation-Failed state's reason copy explicitly names "no problem could be established"
      as its most common cause.
...
- [x] Citation-presence-not-correctness caveat specified as fixed, always-visible copy on the
      Brief review surface, matching architecture's R-1 wording that presence is guaranteed and
      only correctness/relevance is disclaimed (never read as implying citations may be absent).
- [x] Evidence rendering is specified per claim-evidence relationship (`ClaimVersionEvidence`/
      `resolvedEvidence[]`), with stance read from the relationship, not from `EvidenceItem` —
      the same `EvidenceItem` cited by two claims with different stances renders both correctly,
      scoped to its own `ClaimGroup`, never collapsed to one shared badge.
...
- [x] Problem Statement is confirmed non-negatable throughout: no `NegativeFindingNotice`
      rendering path exists for it, `negativeFindings[]` is documented as 0–4 rows (not 0–5),
      and the Completed state's Problem Statement section is stated to always render real
      content by construction (Q-2, binding correction).
