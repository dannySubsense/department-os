...
`docs/specs/product-surface-checkpoint-2/01-REQUIREMENTS.md:452`, with a roadmap fix at
`docs/specs/product-surface-checkpoint-2/04-ROADMAP.md` (slice C2-S1, lines ~229-245). Its
consequence for this audit is worth stating plainly: as the code stands on
...
`docs/specs/product-surface-checkpoint-2/02-ARCHITECTURE.md` §1.4b already describes the
strict-emptiness check rather than the old paywall/JS-shell heuristic, as of this branch's own
2026-09-06 spec-recovery pass — a separate correction from this measurement, not caused by it.

---

## Addendum — provenance of the deletion on this branch

This measurement and its recommended redesign originate on `fix/llm-and-resolver-constant-integrity`
(commit `c0c615e`), where the deletion of `MIN_CONTENT_LENGTH` and the replacement of both call
sites (`resolveSourceArtifact.ts`, `classifyRetrievalOutcome.ts`) with
`body.trim().length === 0` / `bodyLength === 0` was made in the SAME commit as this measurement
artifact — evidence-led, not concurrent-and-convergent. On `recovery/checkpoint-2-from-bfe41c4`,
that commit was cherry-picked directly onto this branch on 2026-09-06, bringing both the
measurement and the code change across together.
